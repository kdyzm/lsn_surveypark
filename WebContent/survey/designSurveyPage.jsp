...
<body>
	...
	<div class="container">
		...
		<table class="table">
			...
			<tr>
				<td class="tdHL">
					<!-- 这里使用sturs2标签直接判断图片是否存在! --> <!-- 在这里加上一个logo标识 --> <s:if
						test="isLogoImageExists()">
						<img width="40px" alt="这是logo标识"
							src="<s:url value='%{logoPath}'/>" />
					</s:if> <s:else>
						<!-- 如果图片不存在，则什么都不显示 -->
					</s:else> <s:property value="title" />
				</td>
				<td class="tdHR"><s:a
						action="SurveyAction_toUploadLogoPage.action" namespace="/">
						<s:param name="surveyId" value="%{#id}"></s:param>
					增加Logo
					</s:a> &nbsp; <s:a action="SurveyAction_toEditSurveyPage.action"
						namespace="/">
						<s:param name="surveyId" value="%{#id}"></s:param>
					编辑调查</s:a>&nbsp; <s:a action="PageAction_toAddPagePage.action"
						namespace="/">
							...
							<div class="panel panel-default">
								...
								<div class="panel-heading" style="padding-left: 20px;">
									...
									<a data-toggle="collapse" data-parent="#accordion"
										href="#<s:property value='%{#page.title}' />"> <s:property
											value="%{#page.title}"/>
									</a>
									<s:a action="PageAction_toEditPageTitlePage.action"
										namespace="/">
										<s:param name="pageId" value="%{#pId}"></s:param>
										<s:param name="surveyId" value="%{#id}"></s:param>
														编辑页面标题</s:a>
									&nbsp;
									<s:a action="PageAction_toSelectTargetPage.action"
										namespace="/">
										<s:param name="pageId" value="%{#pId}"></s:param>
										<s:param name="surveyId" value="%{#id}"></s:param>
														移动/复制页
									...
									&nbsp;
									<s:a action="QuestionAction_toSelectQuestionTypePage.action"
										namespace="/">
										<s:param name="pageId" value="%{#pId}"></s:param>
										<s:param name="surveyId" value="%{#id}"></s:param>
														增加问题</s:a>
								...
								<div id="<s:property value='%{#page.title}' />"
									class="panel-collapse collapse" style="padding-left: 20px;">
									<!-- 迭代问题的集合 -->
									<table class="table">
										<s:iterator value="%{#page.questions}" var="question">
											...
											<tr class="questionTitle">
												...
												<td class="tdHR"><s:a
														action="QuestionAction_editQuestion.action" namespace="/">
														<s:param name="pageId" value="%{#pId}"></s:param>
														<s:param name="surveyId" value="%{#id}"></s:param>
														<s:param name="questionId" value="%{#qid}"></s:param>
																						编辑问题</s:a>&nbsp; <s:a
														action="QuestionAction_deleteQuestion.action"
														namespace="/">
														<s:param name="pageId" value="%{#pId}"></s:param>
														<s:param name="surveyId" value="%{#id}"></s:param>
														<s:param name="questionId" value="%{#qid}"></s:param>
											...
											<tr>
												<td colspan="2">
													...
													</s:if> <!-- 第二种题型,是下拉列表类型的题型 --> <s:elseif test="#qt==4">
														<s:select list="#question.optionTextArr"></s:select>
													</s:elseif> <s:elseif test="#qt==5">
